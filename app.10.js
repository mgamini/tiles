...
Board.prototype.render = function() {
  ...
  for (y = 0; y <= bounds.y.max; y++) {
    output = output.concat([[], []]);
    for (x = 0; x <= bounds.x.max; x++) {
      let tile = this.search('coords', [x,y]);
      // console.log(tile)
      if (tile) {
        // console.log([tile.value[0], tile.value[1]])
        output[y*2] = output[y*2].concat([chalk.green(tile.value[0]), chalk.green(tile.value[1])]);
        output[y*2 + 1] = output[y*2 + 1].concat([chalk.green(tile.value[3]), chalk.green(tile.value[2])]);
      } else {
...
Tile.prototype.render = function() {
  return chalk.green(this.value[0]) + chalk.dim(' | ') + chalk.green(this.value[1]) + '\n' +
    '-----\n' +
    chalk.green(this.value[1]) + chalk.dim(' | ') + chalk.green(this.value[2]);
}
